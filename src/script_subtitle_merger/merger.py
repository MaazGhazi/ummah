...
# Default output directory for merged JSON files
...
def get_default_output_path(movie_title: str) -> Path:
    """Get the default output path for a movie's merged JSON."""
    # Sanitize the movie title for use as filename
    safe_title = re.sub(r'[^\w\s-]', '', movie_title).strip().replace(' ', '_').lower()
    return OUTPUT_DIR / f"{safe_title}_merged.json"
...
class ScriptSubtitleMerger:
    ...
    def merge(
        ...
        low_conf = 0
        unmatched = 0
        entry_index = 1  # Sequential counter starting from 1
        ...
        for sub, matched_dialogue, confidence in matches:
            ...
            entry = MergedEntry(
                index=entry_index,  # Sequential 1-based index
            ...
            entry_index += 1  # Increment for next entry
...
def merge_script_and_subtitles(
    ...
    Args:
        ...
        output_path: Optional path to save JSON output. If None, saves to 
                     src/script_subtitle_merger/output/<movie_title>_merged.json
    ...
    if output_path is None:
        output_path = get_default_output_path(movie_title)
